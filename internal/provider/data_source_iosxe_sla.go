				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"icmp_echo_destination": schema.StringAttribute{
							MarkdownDescription: "Destination IP address or hostname, broadcast disallowed",
							Computed:            true,
						},
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"life": schema.Int64Attribute{
							MarkdownDescription: "Length of time to execute in seconds",
							Computed:            true,
						},
						"start_time_now": schema.BoolAttribute{
							MarkdownDescription: "Start now",
							Computed:            true,
						},
					},
				},
...
func (d *SLADataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if device.Protocol == "restconf" {
		...
		if res.StatusCode == 404 {
			config = SLAData{Device: config.Device}
		} else {
			if err != nil {
