...
func (data BGPPeerPolicyTemplate) addToBodyXML(ctx context.Context, config BGPPeerPolicyTemplate, body netconf.Body) netconf.Body {
	...
	if len(data.RouteMaps) > 0 {
		...
		body = helpers.SetRawFromXPathMulti(body, data.getXPath()+"/route-map1", RouteMapsFragments)
	}
...
func (data *BGPPeerPolicyTemplate) updateFromBodyXML(ctx context.Context, res xmldot.Result) {
	...
	RouteMapsItems := helpers.CollectListItemsXML(RouteMapsParentScope.Raw, "route-map1", RouteMapsKeys[:])
