...
type ClassMap struct {
	...
	MatchActivatedServiceTemplate             []ClassMapMatchActivatedServiceTemplate `tfsdk:"match_activated_service_template"`
...
func (data ClassMap) toBody(ctx context.Context) string {
	...
	if len(data.MatchActivatedServiceTemplate) > 0 {
		body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"match.activated-service-template", []interface{}{})
		for index, item := range data.MatchActivatedServiceTemplate {
...
func (data *ClassMap) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.MatchActivatedServiceTemplate {
		keys := [...]string{"service-name"}
		keyValues := [...]string{data.MatchActivatedServiceTemplate[i].ServiceName.ValueString()}
		...
		if value := r.Get("service-name"); value.Exists() && !data.MatchActivatedServiceTemplate[i].ServiceName.IsNull() {
			data.MatchActivatedServiceTemplate[i].ServiceName = types.StringValue(value.String())
		} else {
			data.MatchActivatedServiceTemplate[i].ServiceName = types.StringNull()
		}
	}
...
func (data *ClassMapData) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "match.activated-service-template"); value.Exists() {
		data.MatchActivatedServiceTemplate = make([]ClassMapMatchActivatedServiceTemplate, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := ClassMapMatchActivatedServiceTemplate{}
			if cValue := v.Get("service-name"); cValue.Exists() {
				item.ServiceName = types.StringValue(cValue.String())
			}
			data.MatchActivatedServiceTemplate = append(data.MatchActivatedServiceTemplate, item)
			return true
		})
	}
...
func (data *ClassMap) getDeletedListItems(ctx context.Context, state ClassMap) []string {
	...
	for i := range state.MatchActivatedServiceTemplate {
		stateKeyValues := [...]string{state.MatchActivatedServiceTemplate[i].ServiceName.ValueString()}

		emptyKeys := true
		if !reflect.ValueOf(state.MatchActivatedServiceTemplate[i].ServiceName.ValueString()).IsZero() {
			emptyKeys = false
		}
		if emptyKeys {
			continue
		...
		found := false
		for j := range data.MatchActivatedServiceTemplate {
			found = true
			if state.MatchActivatedServiceTemplate[i].ServiceName.ValueString() != data.MatchActivatedServiceTemplate[j].ServiceName.ValueString() {
				found = false
			}
			if found {
				break
