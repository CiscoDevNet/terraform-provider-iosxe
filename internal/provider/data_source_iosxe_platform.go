...
func (d *PlatformDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "This data source can read the Platform configuration.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"punt_keepalive_disable_kernel_core": schema.BoolAttribute{
				MarkdownDescription: "Disable IOSXE kernel core generation for keepalive fault",
				Computed:            true,
			},
			"punt_keepalive_settings_fatal_count": schema.Int64Attribute{
				MarkdownDescription: "Set punt-inject keepalive fatal count",
				Computed:            true,
			},
			"punt_keepalive_settings_transmit_interval": schema.Int64Attribute{
				MarkdownDescription: "Set punt-inject keepalive transmit interval",
				Computed:            true,
			},
			"punt_keepalive_settings_warning_count": schema.Int64Attribute{
				MarkdownDescription: "Set punt-inject keepalive warning count",
				Computed:            true,
			},
		},
	}
...
func (d *PlatformDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if device.Protocol == "restconf" {
		...
		if res.StatusCode == 404 {
			config = PlatformData{Device: config.Device}
		} else {
			if err != nil {
