...
func TestAccIosxeStackwiseVirtual(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("iosxe_stackwise_virtual.test", "dual_active_detection_pagp_trust_channel_group", "1"))
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccIosxeStackwiseVirtualConfig_minimum(),
			},
			{
...
func testAccIosxeStackwiseVirtualConfig_minimum() string {
	config := `resource "iosxe_stackwise_virtual" "test" {` + "\n"
	config += `}` + "\n"
	return config
}
...
func testAccIosxeStackwiseVirtualConfig_all() string {
	config := `resource "iosxe_stackwise_virtual" "test" {` + "\n"
	config += `	domain = 10` + "\n"
