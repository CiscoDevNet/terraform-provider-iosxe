...
func (data CryptoIKEv2) toBody(ctx context.Context) string {
	...
	if !data.HttpUrlCert.IsNull() && !data.HttpUrlCert.IsUnknown() {
		if data.HttpUrlCert.ValueBool() {
			body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"http-url.cert", map[string]string{})
		}
	}
	return body
}
...
func (data *CryptoIKEv2) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "http-url.cert"); !data.HttpUrlCert.IsNull() {
		if value.Exists() {
			data.HttpUrlCert = types.BoolValue(true)
		} else {
			data.HttpUrlCert = types.BoolValue(false)
		}
	} else {
...
func (data *CryptoIKEv2) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "http-url.cert"); value.Exists() {
		data.HttpUrlCert = types.BoolValue(true)
	} else {
		data.HttpUrlCert = types.BoolValue(false)
	}
...
func (data *CryptoIKEv2) getEmptyLeafsDelete(ctx context.Context) []string {
	...
	if !data.HttpUrlCert.IsNull() && !data.HttpUrlCert.ValueBool() {
		emptyLeafsDelete = append(emptyLeafsDelete, fmt.Sprintf("%v/http-url/cert", data.getPath()))
	}
