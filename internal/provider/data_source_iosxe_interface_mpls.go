...
var (
	_ datasource.DataSource              = &InterfaceMPLSDataSource{}
	_ datasource.DataSourceWithConfigure = &InterfaceMPLSDataSource{}
)

func NewInterfaceMPLSDataSource() datasource.DataSource {
	return &InterfaceMPLSDataSource{}
}
...
func (d *InterfaceMPLSDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "This data source can read the Interface MPLS configuration.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"ip": schema.BoolAttribute{
				MarkdownDescription: "Configure dynamic MPLS forwarding for IP",
				Computed:            true,
			},
			"mtu": schema.StringAttribute{
				MarkdownDescription: "Set MPLS Maximum Transmission Unit",
				Computed:            true,
			},
		},
	}
...
func (d *InterfaceMPLSDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if res.StatusCode == 404 {
		config = InterfaceMPLSData{Device: config.Device}
	} else {
		if err != nil {
