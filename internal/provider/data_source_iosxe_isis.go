...
func (d *ISISDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "This data source can read the ISIS configuration.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"area_tag": schema.StringAttribute{
				MarkdownDescription: "IS-IS area tag",
				Required:            true,
			},
			"nets": schema.SetNestedAttribute{
				MarkdownDescription: "Network Entity Titles for this IS-IS process",
				Computed:            true,
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						"tag": schema.StringAttribute{
							MarkdownDescription: "NET value",
							Computed:            true,
						},
					},
				},
			...
			"log_adjacency_changes_all": schema.BoolAttribute{
				MarkdownDescription: "Log all adjacency changes including non-IIH events",
				Computed:            true,
			},
		},
	}
...
func (d *ISISDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if device.Protocol == "restconf" {
		...
		if res.StatusCode == 404 {
			config = ISISData{Device: config.Device}
		} else {
			if err != nil {
