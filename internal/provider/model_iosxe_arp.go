...
type ARP struct {
	...
	InspectionFilter                []ARPInspectionFilter `tfsdk:"inspection_filter"`
...
type ARPInspectionFilter struct {
	Arpacl types.String              `tfsdk:"arpacl"`
	Vlan   []ARPInspectionFilterVlan `tfsdk:"vlan"`
}
...
func (data ARP) toBody(ctx context.Context) string {
	...
	if len(data.InspectionFilter) > 0 {
		body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"inspection.filter", []interface{}{})
		for index, item := range data.InspectionFilter {
			if !item.Arpacl.IsNull() && !item.Arpacl.IsUnknown() {
				body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"inspection.filter"+"."+strconv.Itoa(index)+"."+"arpacl", item.Arpacl.ValueString())
			}
			if len(item.Vlan) > 0 {
...
func (data *ARP) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.InspectionFilter {
		keys := [...]string{"arpacl"}
		keyValues := [...]string{data.InspectionFilter[i].Arpacl.ValueString()}
		...
		if value := r.Get("arpacl"); value.Exists() && !data.InspectionFilter[i].Arpacl.IsNull() {
			data.InspectionFilter[i].Arpacl = types.StringValue(value.String())
		} else {
			data.InspectionFilter[i].Arpacl = types.StringNull()
		}
		for ci := range data.InspectionFilter[i].Vlan {
			keys := [...]string{"vlan-range"}
			keyValues := [...]string{data.InspectionFilter[i].Vlan[ci].VlanRange.ValueString()}
			...
			if value := cr.Get("vlan-range"); value.Exists() && !data.InspectionFilter[i].Vlan[ci].VlanRange.IsNull() {
				data.InspectionFilter[i].Vlan[ci].VlanRange = types.StringValue(value.String())
			} else {
				data.InspectionFilter[i].Vlan[ci].VlanRange = types.StringNull()
			}
			if value := cr.Get("static"); !data.InspectionFilter[i].Vlan[ci].Static.IsNull() {
				if value.Exists() {
					data.InspectionFilter[i].Vlan[ci].Static = types.BoolValue(true)
				} else {
					data.InspectionFilter[i].Vlan[ci].Static = types.BoolValue(false)
				}
			} else {
				data.InspectionFilter[i].Vlan[ci].Static = types.BoolNull()
			}
		}
	}
...
func (data *ARPData) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "inspection.filter"); value.Exists() {
		data.InspectionFilter = make([]ARPInspectionFilter, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := ARPInspectionFilter{}
			if cValue := v.Get("arpacl"); cValue.Exists() {
				item.Arpacl = types.StringValue(cValue.String())
			}
			if cValue := v.Get("vlan"); cValue.Exists() {
				item.Vlan = make([]ARPInspectionFilterVlan, 0)
				cValue.ForEach(func(ck, cv gjson.Result) bool {
					cItem := ARPInspectionFilterVlan{}
			...
			data.InspectionFilter = append(data.InspectionFilter, item)
			return true
		})
	}
...
func (data *ARP) getDeletedListItems(ctx context.Context, state ARP) []string {
	...
	for i := range state.InspectionFilter {
		stateKeyValues := [...]string{state.InspectionFilter[i].Arpacl.ValueString()}

		emptyKeys := true
		if !reflect.ValueOf(state.InspectionFilter[i].Arpacl.ValueString()).IsZero() {
			emptyKeys = false
		}
		if emptyKeys {
			continue
		...
		found := false
		for j := range data.InspectionFilter {
			found = true
			if state.InspectionFilter[i].Arpacl.ValueString() != data.InspectionFilter[j].Arpacl.ValueString() {
				found = false
			}
			if found {
				for ci := range state.InspectionFilter[i].Vlan {
					cstateKeyValues := [...]string{state.InspectionFilter[i].Vlan[ci].VlanRange.ValueString()}

					cemptyKeys := true
					if !reflect.ValueOf(state.InspectionFilter[i].Vlan[ci].VlanRange.ValueString()).IsZero() {
						cemptyKeys = false
					}
					if cemptyKeys {
						continue
					...
					found := false
					for cj := range data.InspectionFilter[j].Vlan {
						found = true
						if state.InspectionFilter[i].Vlan[ci].VlanRange.ValueString() != data.InspectionFilter[j].Vlan[cj].VlanRange.ValueString() {
							found = false
						}
						if found {
							break
...
func (data *ARP) getEmptyLeafsDelete(ctx context.Context) []string {
	...
	for i := range data.InspectionFilter {
		...
		for ci := range data.InspectionFilter[i].Vlan {
			ckeyValues := [...]string{data.InspectionFilter[i].Vlan[ci].VlanRange.ValueString()}
			if !data.InspectionFilter[i].Vlan[ci].Static.IsNull() && !data.InspectionFilter[i].Vlan[ci].Static.ValueBool() {
...
func (data *ARP) getDeletePaths(ctx context.Context) []string {
	...
	if !data.InspectionValidateSrcMac.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/validate/src-mac", data.getPath()))
	}
	if !data.InspectionValidateDstMac.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/validate/dst-mac", data.getPath()))
	}
	if !data.InspectionValidateIp.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/validate/ip", data.getPath()))
	}
	if !data.InspectionValidateAllowZeros.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/validate/allow/zeros", data.getPath()))
	}
	...
	if !data.InspectionLogBufferLogsEntries.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/log-buffer/logs/entries", data.getPath()))
	}
	if !data.InspectionLogBufferLogsInterval.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/inspection/log-buffer/logs/interval", data.getPath()))
	}
