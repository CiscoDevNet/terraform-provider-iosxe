...
type Radius struct {
	...
	AddressAcctPort                                       types.Int64  `tfsdk:"address_acct_port"`
	...
	AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig types.Bool   `tfsdk:"automate_tester_type_of_testing_probe_on_config_probe_on_config"`
	PacKeyKey                                             types.String `tfsdk:"pac_key_key"`
...
func (data Radius) toBody(ctx context.Context) string {
	...
	if !data.AddressAcctPort.IsNull() && !data.AddressAcctPort.IsUnknown() {
		body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"address.acct-port", strconv.FormatInt(data.AddressAcctPort.ValueInt64(), 10))
	}
	...
	if !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.IsNull() && !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.IsUnknown() {
		if data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.ValueBool() {
			body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"automate-tester.probe-on-config", map[string]string{})
		}
	}
	if !data.PacKeyKey.IsNull() && !data.PacKeyKey.IsUnknown() {
		body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"pac.key.key", data.PacKeyKey.ValueString())
	}
...
func (data *Radius) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "address.acct-port"); value.Exists() && !data.AddressAcctPort.IsNull() {
		data.AddressAcctPort = types.Int64Value(value.Int())
	} else {
		data.AddressAcctPort = types.Int64Null()
	}
	...
	if value := res.Get(prefix + "automate-tester.probe-on-config"); !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.IsNull() {
		if value.Exists() {
			data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig = types.BoolValue(true)
		} else {
			data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig = types.BoolValue(false)
		}
	} else {
		data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig = types.BoolNull()
	}
	if value := res.Get(prefix + "pac.key.key"); value.Exists() && !data.PacKeyKey.IsNull() {
		data.PacKeyKey = types.StringValue(value.String())
	} else {
		data.PacKeyKey = types.StringNull()
	}
...
func (data *Radius) getEmptyLeafsDelete(ctx context.Context) []string {
	...
	if !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.IsNull() && !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.ValueBool() {
...
func (data *Radius) getDeletePaths(ctx context.Context) []string {
	...
	if !data.AddressAcctPort.IsNull() {
	...
	if !data.AutomateTesterTypeOfTestingProbeOnConfigProbeOnConfig.IsNull() {
		deletePaths = append(deletePaths, fmt.Sprintf("%v/automate-tester/probe-on-config", data.getPath()))
	}
	if !data.PacKeyKey.IsNull() {
