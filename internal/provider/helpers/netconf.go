...
// RemoveFromXPathMulti batches multiple RemoveFromXPath calls into a single
// read+write pass on the body, instead of one read+write per xPath.
//
// Background: RemoveFromXPath (via appendSiblingElement) handles a new list
// sibling by reading the ENTIRE existing content at the parent path, string-
// concatenating the new element, and writing the whole thing back with SetRaw.
// It also calls augmentNamespaces, which -- once more than one sibling exists --
// loops over every existing sibling to check/set its xmlns attribute. Both of
// these costs scale with the number of siblings already present, so calling
// RemoveFromXPath once per item in a loop (the pattern generated by
// addDeletedItemsXML for removed list entries, e.g. removed VLANs, ACEs,
// neighbors, etc.) is O(n^2) in the number of items. This function does the
// same work but with exactly one read, one write, and one augmentNamespaces
// call for the whole batch, giving O(n) instead.
//
// Callers don't need to guarantee anything about the input: xPaths that don't
// share a common list parent (or that don't contain a list-key predicate at
// all) are simply processed one at a time via RemoveFromXPath, so correctness
// never depends on the batching assumption -- only the performance win does.
// The one assumption that DOES need to hold for two xPaths to land in the same
// batch is that they must be genuinely new, non-duplicate siblings (as is the
// case when a caller loops over a list of removed keys) -- this function does
// not check for an existing element with matching keys the way findSiblingInfo
// does for a single call, so it should not be used where "update this existing
// element instead of appending" is a real possibility within the batch.
func RemoveFromXPathMulti(body netconf.Body, xPaths []string) netconf.Body {
	if len(xPaths) == 0 {
		return body
	}
	if len(xPaths) == 1 {
		return RemoveFromXPath(body, xPaths[0])
	}
	...
	for _, xPath := range xPaths {
		...
		// No list-key predicate anywhere in the path -- nothing to batch,
		// there's no growing sibling list this could be O(n^2) against.
		if listSegIdx == -1 {
			body = RemoveFromXPath(body, xPath)
			continue
		}

		prefix := strings.Join(segments[:listSegIdx], "/")
		shapeParts := make([]string, 0, len(segments)-listSegIdx)
		for _, seg := range segments[listSegIdx:] {
			name, keys := parseXPathSegment(seg)
			shapeParts = append(shapeParts, fmt.Sprintf("%s(%d)", removeNamespacePrefix(name), len(keys)))
		}
		groupKey := prefix + "||" + strings.Join(shapeParts, "/")

		if _, ok := groups[groupKey]; !ok {
			order = append(order, groupKey)
		}
		groups[groupKey] = append(groups[groupKey], parsedPath{xPath: xPath, segments: segments, listSegIdx: listSegIdx})
	}

	for _, groupKey := range order {
		items := groups[groupKey]
		if len(items) == 1 {
			body = RemoveFromXPath(body, items[0].xPath)
			continue
		}

		first := items[0]
		prefixSegments := first.segments[:first.listSegIdx]

		// Build the clean (namespace-stripped) dot-path for the shared parent
		// directly, rather than via buildXPathStructure(ensureStructure=true):
		// that helper decides whether the path "already exists" by reading its
		// *text value* (.String()), which is empty for a container that holds
		// only child elements (e.g. a sibling like /logging already appended
		// under the same parent) -- indistinguishable from "doesn't exist yet".
		// That false negative causes it to Set the container to "", wiping any
		// content already there. body.SetRaw below auto-creates the parent
		// structure if it's genuinely absent, same as the single-item
		// appendSiblingElement path does, so no separate "ensure" step is needed.
		cleanPrefix := make([]string, 0, len(prefixSegments))
		for _, seg := range prefixSegments {
			name, _ := parseXPathSegment(seg)
			cleanPrefix = append(cleanPrefix, removeNamespacePrefix(name))
		}
		parentPath := strings.Join(cleanPrefix, ".")

		var existingContent string
		if parentPath != "" {
			existingContent = xmldot.Get(body.Res(), parentPath).Raw
		} else {
			existingContent = body.Res()
		}

		var newFragments strings.Builder
		for _, item := range items {
			remainingSegments := item.segments[item.listSegIdx:]
			newFragments.WriteString(buildRemoveFragment(remainingSegments))
		}

		combined := existingContent + newFragments.String()
		if parentPath != "" {
			body = body.SetRaw(parentPath, combined)
		} else {
			body = netconf.NewBody(combined)
		}

		// Augment namespaces on the ancestor segments only (containers like
		// "native"). Those are singletons, so augmentNamespaces' counted lookup
		// is safe there. Everything from the list segment down already carries
		// an inline xmlns from buildRemoveFragment -- deliberately, because
		// augmentNamespaces would silently skip siblings past xmldot's
		// MaxWildcardResults (1000) cap and produce elements the device rejects
		// as unknown-element.
		if len(prefixSegments) > 0 {
			ancestorDotPath := strings.Join(prefixSegments, ".")
			body = augmentNamespaces(body, ancestorDotPath)
		}
	}
...
// buildRemoveFragment builds the nested XML for a chain of XPath segments,
// rendering list-key predicates as child elements the way appendSiblingElement
// does, and marking the deepest (last) element with operation="remove". It is
// the allocation-only counterpart to appendSiblingElement: it never touches a
// shared netconf.Body, so many fragments can be built independently and then
// concatenated and written to the body once (see RemoveFromXPathMulti).
//
// Any segment carrying a namespace prefix gets its xmlns written directly into
// the emitted element. This is deliberate and must not be deferred to
// augmentNamespaces: that function discovers repeated siblings through xmldot's
// counted/indexed queries, which cap out at MaxWildcardResults (1000), so for a
// list longer than 1000 it silently leaves every element past the 1000th
// without an xmlns. The device then resolves those elements against the parent
// (native) namespace and rejects the whole edit-config with unknown-element.
// Baking the attribute in here is also cheaper: it is O(1) per fragment with no
// re-scan of the growing document.
func buildRemoveFragment(segments []string) string {
	if len(segments) == 0 {
		return ""
	}

	elementName, keys := parseXPathSegment(segments[0])
	cleanName := removeNamespacePrefix(elementName)

	// Resolve this segment's namespace (if it is prefixed) and render it as an
	// inline attribute on the opening tag.
	openAttrs := ""
	if idx := strings.Index(elementName, ":"); idx != -1 {
		prefix := elementName[:idx]
		ns, ok := namespaceExceptions[prefix]
		if !ok {
			ns = namespaceBaseURL + prefix
		}
		openAttrs = fmt.Sprintf(` xmlns="%s"`, ns)
	}

	var inner strings.Builder
	for _, kv := range keys {
		if kv.Key == "." {
			inner.WriteString(html.EscapeString(kv.Value))
		} else {
			inner.WriteString(fmt.Sprintf("<%s>%s</%s>", kv.Key, html.EscapeString(kv.Value), kv.Key))
		}
	}

	if len(segments) == 1 {
		return fmt.Sprintf(`<%s%s operation="remove">%s</%s>`, cleanName, openAttrs, inner.String(), cleanName)
	}

	inner.WriteString(buildRemoveFragment(segments[1:]))
	return fmt.Sprintf("<%s%s>%s</%s>", cleanName, openAttrs, inner.String(), cleanName)
}
...
// SetRawFromXPathMulti inserts multiple sibling elements at xPath in a single pass.
// It is equivalent to calling SetRawFromXPath once per entry in values, in order,
// but does so without re-parsing and rebuilding the body on every entry.
//
// SetRawFromXPath supports inserting repeated sibling elements (e.g. multiple
// <vlan> entries in a list) by being called once per item. Each call reads the
// current body, appends one more wrapped element, and rewrites the body - so
// building a list of n items costs n full body rebuilds, which is O(n^2) overall
// (confirmed by xmldot's own documented complexity for repeated Set operations
// against a growing document). This function performs the equivalent wrapping
// for every item up front (cheap: each wrap is independent of the others), then
// does exactly one existing-content read and one write, regardless of n.
//
// Only the multi-segment path form (xPath with a parent, e.g.
// ".../native/interface/description-list") is optimized here, since that is the
// form used by generated list-attribute code (data.getXPath() always yields at
// least one path segment). If a single-segment path is passed, this falls back
// to calling SetRawFromXPath in a loop so behavior still matches exactly.
func SetRawFromXPathMulti(body netconf.Body, xPath string, values []string) netconf.Body {
	// Drop empty values up front; SetRawFromXPath is a no-op for these.
	...
	if len(filtered) == 1 {
		return SetRawFromXPath(body, xPath, filtered[0])
	}

	trimmedPath := strings.TrimPrefix(xPath, "/")
	segments := splitXPathSegments(trimmedPath)
	if len(segments) <= 1 {
		// Rare/unused-by-generated-code shape: fall back to the safe,
		// already-correct (if quadratic) per-item behavior.
		for _, v := range filtered {
			body = SetRawFromXPath(body, xPath, v)
		}
		return body
	}

	finalSegment := segments[len(segments)-1]
	finalElement, _ := parseXPathSegment(finalSegment)
	finalElementClean := removeNamespacePrefix(finalElement)

	// Resolve the namespace for the final segment up front (if any), so it
	// can be embedded directly into each fragment below. Doing it this way -
	// baking xmlns into each fragment before the single combined write -
	// avoids needing any indexed Set() calls afterward. Each Set() by index
	// has to linear-scan to that index and rebuild the whole document, so
	// doing one per item (as an earlier version of this function did) is
	// itself an O(n) cost per item on top of the O(n) items - quadratic
	// overall, for work that embedding the attribute up front avoids
	// entirely.
	finalNamespace := ""
	if idx := strings.Index(finalSegment, ":"); idx != -1 {
		prefix := finalSegment[:idx]
		ns, ok := namespaceExceptions[prefix]
		if !ok {
			ns = namespaceBaseURL + prefix
		}
	...
	// Wrap every item once - O(n) total, and independent per item.
	wrappedFragments := make([]string, 0, len(filtered))
	for _, v := range filtered {
		if finalNamespace != "" {
			wrappedFragments = append(wrappedFragments, "<"+finalElementClean+" xmlns=\""+finalNamespace+"\">"+v+"</"+finalElementClean+">")
		} else {
			wrappedFragments = append(wrappedFragments, "<"+finalElementClean+">"+v+"</"+finalElementClean+">")
		}
	}
	joinedContent := strings.Join(wrappedFragments, "")
	...
	// Single read of existing content, single write of the combined result -
	// this is the part that used to happen once per item.
	existingXML := xmldot.Get(body.Res(), parentPath).Raw
	if existingXML != "" {
		body = body.SetRaw(parentPath, existingXML+joinedContent)
	} else {
		body = body.SetRaw(parentPath, joinedContent)
	}

	// Augment namespaces on the ancestor segments (containers like "native",
	// "spanning-tree") the normal way - these are always singleton elements,
	// so there's no array-size concern there. The final (list) segment's
	// namespace is already embedded in each fragment above, so it needs no
	// further handling here.
	if len(segments) > 1 {
		ancestorDotPath := strings.Join(segments[:len(segments)-1], ".")
		body = augmentNamespaces(body, ancestorDotPath)
	}
...
// CompositeKey joins one or more key values into a single string suitable
// for use as a map key (see CollectListItemsXML), using a separator
// character unlikely to appear in real configuration values.
func CompositeKey(values ...string) string {
	return strings.Join(values, "\x1f")
}

// CollectListItemsXML walks rawXML exactly once, collecting every
// occurrence of a repeated element into a map keyed by CompositeKey of its
// key value(s), without going through xmldot's capped array/count/index
// queries.
//
// GetFromXPath (and the wildcard/count/indexed forms of xmldot.Get it uses
// to detect and return repeated siblings) has to collect every matching
// element into an array before it can count or index into them, and xmldot
// caps that collection at MaxWildcardResults (1000) as a DoS protection.
// For a list with more than 1000 items, every element past that limit
// becomes invisible to those queries - not an error, just silently absent -
// so relying on GetFromXPath to locate items in a list bigger than 1000
// silently drops state for anything past the 1000th.
//
// This walks the raw XML directly instead of collecting an array: for the
// final (repeated) path segment, it repeatedly asks xmldot for just the
// first element with that name (an uncapped, single-match lookup - the cap
// only applies to "how many"/"which index" queries, not "give me the first
// one"), records it, deletes that one element from a scratch copy, and asks
// again. There's no array to cap, so it can walk arbitrarily many siblings.
// Doing this once for the whole list and reusing the result via O(1) map
// lookups (rather than repeating the walk once per item being looked up)
// keeps the total cost to one pass over the list rather than compounding
// per-lookup costs into quadratic or worse behavior for large lists.
func CollectListItemsXML(rawXML string, elementPath string, keyNames []string) map[string]xmldot.Result {
	result := make(map[string]xmldot.Result)

	trimmedPath := strings.TrimPrefix(elementPath, "/")
	segments := splitXPathSegments(trimmedPath)
	if len(segments) == 0 {
		return result
	}
	...
	if len(segments) > 1 {
		for _, seg := range segments[:len(segments)-1] {
			...
			parent := xmldot.Get(remaining, name)
			if !parent.Exists() {
				return result
			}
			remaining = parent.Raw
		}
	}

	finalSegment := segments[len(segments)-1]
	elementName, _ := parseXPathSegment(finalSegment)
	elementName = removeNamespacePrefix(elementName)

	for {
		match := xmldot.Get(remaining, elementName)
		if !match.Exists() {
			break
		}
		keyParts := make([]string, len(keyNames))
		for i, kn := range keyNames {
			keyParts[i] = match.Get(removeNamespacePrefix(kn)).String()
		}
		result[CompositeKey(keyParts...)] = match

		next, err := xmldot.Delete(remaining, elementName)
		if err != nil || next == remaining {
			// Can't make progress - stop rather than loop forever.
			break
		}
		remaining = next
	}
