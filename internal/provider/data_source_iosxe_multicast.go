...
var (
	_ datasource.DataSource              = &MulticastDataSource{}
	_ datasource.DataSourceWithConfigure = &MulticastDataSource{}
)
...
func (d *MulticastDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "This data source can read the Multicast configuration.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"multipath": schema.BoolAttribute{
				MarkdownDescription: "RPF across equal-cost paths",
				Computed:            true,
			},
			"multipath_s_g_hash": schema.StringAttribute{
				MarkdownDescription: "Per-(source, group) load sharing algorithm selection",
				Computed:            true,
			},
...
func (d *MulticastDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if device.Protocol == "restconf" {
		...
		if res.StatusCode == 404 {
			config = MulticastData{Device: config.Device}
		} else {
			if err != nil {
