...
func (data AccessListIPv6) addToBodyXML(ctx context.Context, config AccessListIPv6, body netconf.Body) netconf.Body {
	...
	if len(data.Entries) > 0 {
		...
		for _, item := range data.Entries {
			...
			if !item.ServiceObjectGroup.IsNull() && !item.ServiceObjectGroup.IsUnknown() {
				cBody = helpers.SetFromXPath(cBody, "ace-rule/object-group", item.ServiceObjectGroup.ValueString())
			}
			...
			if !item.SourceFqdnGroup.IsNull() && !item.SourceFqdnGroup.IsUnknown() {
				cBody = helpers.SetFromXPath(cBody, "ace-rule/source-fqdn-group", item.SourceFqdnGroup.ValueString())
			}
			...
			if !item.DestinationFqdnGroup.IsNull() && !item.DestinationFqdnGroup.IsUnknown() {
				cBody = helpers.SetFromXPath(cBody, "ace-rule/destination-fqdn-group", item.DestinationFqdnGroup.ValueString())
			}
...
func (data *AccessListIPv6) updateFromBodyXML(ctx context.Context, res xmldot.Result) {
	...
	for i := range data.Entries {
		...
		if value := helpers.GetFromXPath(r, "ace-rule/object-group"); value.Exists() && !data.Entries[i].ServiceObjectGroup.IsNull() {
		...
		if value := helpers.GetFromXPath(r, "ace-rule/source-fqdn-group"); value.Exists() && !data.Entries[i].SourceFqdnGroup.IsNull() {
		...
		if value := helpers.GetFromXPath(r, "ace-rule/destination-fqdn-group"); value.Exists() && !data.Entries[i].DestinationFqdnGroup.IsNull() {
...
func (data *AccessListIPv6) fromBodyXML(ctx context.Context, res xmldot.Result) {
	if value := helpers.GetFromXPath(res, "data"+data.getXPath()+"/access-list-seq-rule"); value.Exists() {
		...
		value.ForEach(func(_ int, v xmldot.Result) bool {
			...
			if cValue := helpers.GetFromXPath(v, "ace-rule/source-fqdn-group"); cValue.Exists() {
			...
			if cValue := helpers.GetFromXPath(v, "ace-rule/destination-fqdn-group"); cValue.Exists() {
		...
		for j := range data.Entries {
			...
			if found {
				...
				if !state.Entries[i].DestinationFqdnGroup.IsNull() && data.Entries[j].DestinationFqdnGroup.IsNull() {
					b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/access-list-seq-rule%v/ace-rule/destination-fqdn-group", predicates))
				}
				...
				if !state.Entries[i].SourceFqdnGroup.IsNull() && data.Entries[j].SourceFqdnGroup.IsNull() {
					b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/access-list-seq-rule%v/ace-rule/source-fqdn-group", predicates))
				}
