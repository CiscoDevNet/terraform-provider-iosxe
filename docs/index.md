...
- `read_cache` (Boolean) Enable caching of NETCONF read operations. When enabled, the provider fetches the complete device configuration once on the first read and serves subsequent read operations from the cache. The cache is automatically invalidated after any write operation (Create/Update/Delete). Only applies to NETCONF protocol. **Note**: This is experimental and may improve performance only for very large deployments (1000+ resources). For smaller deployments, individual filtered queries are typically faster. This can also be set as the IOSXE_READ_CACHE environment variable. Defaults to `false`.
