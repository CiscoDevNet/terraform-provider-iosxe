...
- Fix code generator not wiring `read_filter` into generated model code, causing `iosxe_route_map` `set_communities` to return raw decimal integers instead of normalized AA:NN community values on state refresh, resulting in perpetual plan drift
- BREAKING CHANGE: Remove all community `_legacy` attributes (`set_communities_legacy`, `set_communities_additive_legacy`, `set_community_none_legacy`, `set_community_list_delete_legacy`, `set_community_list_standard_legacy`, `set_community_list_expanded_legacy`, `set_community_list_name_legacy`, `match_community_lists_legacy`, `match_extcommunity_lists_legacy`) from `iosxe_route_map` resource and data source. The gen-1 attributes used the `set/community/...` YANG path (obsolete since IOS-XE 17.15). The gen-2 `set_communities` and `set_communities_additive` attributes used the deprecated `community-list` YANG leaf, which cannot round-trip on 17.15 devices (writes to `community-list` but reads back under `community-list-v2`, causing perpetual drift). Replace `set_communities` and `set_communities_additive` with new attributes targeting `community-list-v2` for IOS-XE 17.15+
>>>>>>> d5abc498 (fix(route_map): migrate set_communities to community-list-v2 YANG path)
...
- Add `match_protocol` and `match_class_map` attributes to `iosxe_class_map` resource and data source for Zone-Based Firewall (ZBF) inspect class-map configuration
