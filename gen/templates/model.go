...
func (data {{camelCase .Name}}) getXPath() string {
	path := helpers.ConvertRestconfPathToXPath("{{.Path}}")
{{- if hasId .Attributes}}
	path = fmt.Sprintf(path{{range .Attributes}}{{if .Reference}}, url.QueryEscape(fmt.Sprintf("%v", data.{{toGoName .TfName}}.Value{{.Type}}())){{end}}{{end}}{{range .Attributes}}{{if .Id}}, "{{.YangName}}", url.QueryEscape(fmt.Sprintf("%v", data.{{toGoName .TfName}}.Value{{.Type}}())){{end}}{{end}})
{{- end}}
	return path
}
...
func (data {{camelCase .Name}}) toBodyXML(ctx context.Context) string {
	...
	if !data.{{toGoName .TfName}}.IsNull() && !data.{{toGoName .TfName}}.IsUnknown() {
		...
		var values []string
		data.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
		body = helpers.SetFromXPath(body, data.getXPath() + "/{{.XPath}}", values)
		{{- else if or (eq .Type "Int64List") (eq .Type "Int64Set")}}
		var values []int
		data.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
		body = helpers.SetFromXPath(body, data.getXPath() + "/{{.XPath}}", values)
	...
	if len(data.{{toGoName .TfName}}) > 0 {
		for _, item := range data.{{toGoName .TfName}} {
			...
			if !item.{{toGoName .TfName}}.IsNull() && !item.{{toGoName .TfName}}.IsUnknown() {
				...
				var values []string
				item.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
				cBody = helpers.SetFromXPath(cBody, "{{.XPath}}", values)
				{{- else if or (eq .Type "Int64List") (eq .Type "Int64Set")}}
				var values []int
				item.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
				cBody = helpers.SetFromXPath(cBody, "{{.XPath}}", values)
			...
			if len(item.{{toGoName .TfName}}) > 0 {
				for _, citem := range item.{{toGoName .TfName}} {
					...
					if !citem.{{toGoName .TfName}}.IsNull() && !citem.{{toGoName .TfName}}.IsUnknown() {
						...
						var values []string
						citem.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
						ccBody = helpers.SetFromXPath(ccBody, "{{.XPath}}", values)
						{{- else if or (eq .Type "Int64List") (eq .Type "Int64Set")}}
						var values []int
						citem.{{toGoName .TfName}}.ElementsAs(ctx, &values, false)
						ccBody = helpers.SetFromXPath(ccBody, "{{.XPath}}", values)
...
func (data *{{camelCase .Name}}) addDeletedItemsXML(ctx context.Context, state {{camelCase .Name}}, body string) string {
	...
	if !state.{{toGoName .TfName}}.IsNull() {
		if data.{{toGoName .TfName}}.IsNull() {
			...
			for _, v := range stateValues {
				...
				if !found {
					b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/{{getDeletePath .}}[.=%v]", v))
				}
			}
		}
	}
	...
	for i := range state.{{toGoName .TfName}} {
		...
		for j := range data.{{toGoName .TfName}} {
			...
			if found {
				...
				if !state.{{$list}}[i].{{toGoName .TfName}}.IsNull() {
					if data.{{$list}}[j].{{toGoName .TfName}}.IsNull() {
						...
						for _, v := range stateValues {
							...
							if !found {
								b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/{{$xpath}}%v/{{getDeletePath .}}[.=%v]", predicates, v))
							}
						}
					}
				}
				...
				for ci := range state.{{$list}}[i].{{toGoName .TfName}} {
							...
							if !state.{{$list}}[i].{{$clist}}[ci].{{toGoName .TfName}}.IsNull() {
								if data.{{$list}}[j].{{$clist}}[cj].{{toGoName .TfName}}.IsNull() {
									...
									for _, v := range stateValues {
										...
										if !found {
											b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/{{$xpath}}%v/{{$cXpath}}%v/{{getDeletePath .}}=[.%v]", predicates, cpredicates, v))
										}
									}
								}
							}
					...
					if !found {
						b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/{{$xpath}}%v/{{getDeletePath .}}%v", predicates, cpredicates))
					}
				}
				{{- end}}
				{{- end}}
		...
		if !found {
			b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/{{getDeletePath .}}%v", predicates))
		}
	}
	{{- end}}
	{{- end}}
