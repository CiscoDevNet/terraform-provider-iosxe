...
func TestAccDataSourceIosxe{{camelCase .Name}}(t *testing.T) {
	...
	{{- $name := .Name }}
	{{- range .Attributes}}
	{{- if and (not .Id) (not .Reference) (not .WriteOnly) (not .ExcludeTest) (not .ExampleOnly) (ne .Type "StringSet") (ne .Type "Int64Set") }}
	...
	{{- end}}
	{{- range .Attributes}}
	{{- if and (not .WriteOnly) (not .ExcludeTest) (not .ExampleOnly) (ne .Type "StringSet") (ne .Type "Int64Set")}}
	{{- if or (eq .Type "List") (eq .Type "Set")}}
	{{- $clist := .TfName }}
	{{- if len .TestTags}}
	if {{range $i, $e := .TestTags}}{{if $i}} || {{end}}os.Getenv("{{$e}}") != ""{{end}} {
...
func testAccDataSourceIosxe{{camelCase .Name}}Config() string {
	...
	{{- end}}
	{{- range  .Attributes}}
	{{- if and (not .ExcludeTest) (not .ExampleOnly)}}
	{{- if or (eq .Type "List") (eq .Type "Set")}}
	{{- if len .TestTags}}
	if {{range $i, $e := .TestTags}}{{if $i}} || {{end}}os.Getenv("{{$e}}") != ""{{end}} {
	{{- end}}
